# -*- coding: utf-8 -*-
"""Location: ./plugins/tt_pii_filter/tt_pii_filter.py
Copyright 2025
...
class PIIFilterConfig(BaseModel):
    ...
    detect_tt_type: bool = Field(default=True, description="Detect TT Type identifiers (format: s373-2312-r543)")
...
class PIIDetector:
    ...
    def _compile_patterns(self) -> None:
        ...
        # TT Type patterns - format: letter + 3 digits - 4 digits - letter + 3 digits (e.g., s373-2312-r543)
        if self.config.detect_tt_type:
            patterns.append(
                PIIPattern(
                    type=PIIType.TT_TYPE,
                    pattern=r'\b[a-zA-Z]\d{3}-\d{4}-[a-zA-Z]\d{3}\b',
                    description="TT Type identifier",
                    mask_strategy=MaskingStrategy.PARTIAL,
                )
            )
    ...
    def _apply_mask(self, value: str, pii_type: PIIType, strategy: MaskingStrategy) -> str:  # noqa: PLR0911
        ...
        if strategy == MaskingStrategy.REDACT:
            ...
            elif pii_type == PIIType.TT_TYPE:
                # TT Type: Show last 4 characters for identification
                if len(value) >= 4:
                    return f"****-****-{value[-4:]}"
                return self.config.redaction_text
...
class TTPIIFilterPlugin(Plugin):
